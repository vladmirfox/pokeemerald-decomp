    ...
    switch (sMiningUiState->loadGameState)
    {
        ...
        case 1:
            if (FreeTempTileDataBuffersIfPossible() != TRUE)
            {
                ...
                DecompressDataWithHeaderWram(gStressLevelAndTerrainTiles, sMiningUiState->sBg2TilemapBuffer);
                //DecompressDataWithHeaderWram(sUiTilemap, sMiningUiState->sBg3TilemapBuffer);
                DecompressDataWithHeaderWram(sUiTiles, sMiningUiState->sBg3TilemapBuffer);
                sMiningUiState->loadGameState++;
            }
            break;
        case 2:
